...
- **🧭 Scout — lean-context mode** *(experimental)* — a toggle in the composer, just left of the model picker, that hands heavy exploration — surveying a subsystem, auditing, or tracing something across many files — to an isolated helper agent running your **same model**. It reads in its own throwaway context and reports back only a summary, so big file reads never flood your main conversation (or get re-billed against it every turn) — keeping long sessions noticeably leaner on tokens. **Off by default**; flip it on for big-codebase questions and it applies from your next message.

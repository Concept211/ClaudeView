...
- **💬 Chat with Claude Code** — streamed Markdown (with **tables rendered as a clean bordered grid**), **tool calls — each with its own color-coded icon** (Bash, Grep, Read, Edit, subagents, and more), tool results, and live to-do checklists. **A subagent's own tool calls nest under it** so you can follow what a delegated agent is doing. **File paths Claude mentions are clickable**, and **mockups it builds for you open automatically in a fast built-in preview** — an in-app browser window with **Reload** and **Open in browser** that shrugs off a page crash and reloads itself. Every mockup also drops a one-click **Open mockup** link in the transcript, and one written while you're viewing another session opens the moment you switch back — it never gets lost. A tray menu picks how widely it triggers — **only `_mockups` folders · any mockup-named folder · any HTML file · off** — or you can send mockups to your default browser instead; other URLs open in your default browser too. Every code block gets a hover **Copy** button, every response gets a small **copy / pin / jump-to-prompt** action row, and **your own messages get copy *and* edit buttons** — Claude.ai-style throughout. **Edit a message you've already sent to rewind the conversation to that point and re-run from your revised wording** — the earlier branch is copied, not clobbered, so nothing is lost server-side.
- **📌 Pin & recall messages** — pin any message in a session (yours or Claude's) to bookmark it, then jump straight back from a compact **Pinned** popover at the top of the chat: one click scrolls it into view and flashes it. Pinned messages carry a subtle accent marker inline so they're easy to spot while scrolling, and your pins **persist per session across restarts**.
...
- **🍃 Eco — intelligent token-savings mode** — a one-tap leaf toggle in the composer, just left of the model picker: a **master switch over a suite of savers** you can also tune one by one. It **hands heavy multi-file exploration to an isolated same-model subagent** that reads in its own throwaway context and reports back only a summary (so big reads never flood your main conversation or get re-billed against it every turn), **eases reasoning effort on trivial turns** and restores your full effort for the next real one (a green ring shows when it's eased), **auto-compacts** the moment a turn tips the context over the cliff, **trims external MCP tools** — Gmail · Drive · Calendar and the like — you aren't using this session, and offers a **lossless "start fresh" nudge** when a big context meets a brand-new task. **Off by default**; flip it on and it applies from your next message. (Replaces the earlier Scout toggle, now folded in as one of its behaviors.)
...
- **📋 Plan mode, clickable** — `AskUserQuestion` renders as selectable option cards, and — just like Anthropic's official VS Code extension — **every question also carries an "Other" field so you can type your own answer when none of the choices fit** (mix a typed answer on one question with picks on the rest); `ExitPlanMode` as an **Approve / Reject** card. Reject → edit → re-present works end to end. The cards **wait as long as you need** — take an hour to decide and the turn stays parked for your answer, never timing out behind your back.
...
- **🔀 Git panel** — status, colored changes, history, and one-box commit (Enter to commit); auto-refreshes after each turn. **Click a changed file to reveal it in the tree and open its diff** — a tinted, side-bar diff view with colored `+`/`−` lines. It also **spots a placeholder commit identity** (like `Your Name <you@example.com>`) and offers to set your real name and email in one click — pre-filled from your signed-in Claude account, for this repo or globally — so your commits are attributed correctly. It also **opens repos whose folder is owned by a different Windows account** — say, one cloned by an admin under a shared web root — that trip up stricter Git libraries into "not a git repository." Not a repo yet? **Initialize one in a click.**
...
- **🪟 Polished window** — a **responsive collapsing layout** that folds the side panels away as it narrows (down to a phone-width single column), with its **own chat font size for the compact view** kept separate from your desktop size. It restores its size/maximized state, scroll-follows-cursor across panels, and **double-click any panel splitter to reset it**. **Collapse a side panel outright by dragging it past its minimum width or right-clicking its header, then bring it back from the title bar** — the chat fills the freed space, with no empty gap left behind. Panel widths, collapsed state, *and* the Explorer/Git and Sessions/To-Do splits are all **remembered across sessions and restarts**.

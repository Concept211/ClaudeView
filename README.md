...
- **✅ Live To-Do panel** — when Claude tracks its work as a task list, it gets its own panel pinned under Sessions and **mirrors that list in real time**: items check off as Claude completes them, topped with an *X of Y done* progress bar. It **appears only when there's a list** (and hands the space back to Sessions otherwise), and the divider between the two **drags to resize · double-clicks to reset**, remembered across restarts.
